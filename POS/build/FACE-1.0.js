...
var FACE = function () {
  // Private Section
  var K_VERSION = "1.0";
  var K_SERVICE_URL = "http://facerec-prod.apigee.net/face-track/api/detect/";
  ...
  var sendForm = function sendForm(formData, onSuccessCallback, onFailureCallback) {
  ...
  return {
    ...
    sendImage: function sendImage(imgFile, onSuccessCallback, onFailureCallback, api_key, client_id, attribute) {

      if (!imgFile || !FACE.util.isImage(imgFile)) {
        if (onFailureCallback) onFailureCallback("No file selected or file is not an image");
        return;
      }
      ...
      if (attribute) formData.append(K_FORM_ATTRIBUTE_FIELD_NAME, attribute);

      sendForm(formData, onSuccessCallback, onFailureCallback);
    },

    sendUrl: function sendUrl(url, onSuccessCallback, onFailureCallback, api_key, client_id, attribute) {
    ...
    webcam: function () {
      return {
        startPlaying: function startPlaying(videoTagID) {
          if (!videoTagID || typeof videoTagID != 'string') return;
        ...
        stopPlaying: function stopPlaying(videoTagID) {
          if (!videoTagID || typeof videoTagID != 'string') return;

          var video = document.getElementById(videoTagID);
          video.pause();
          if (navigator.mozGetUserMedia) video.mozSrcObject = null;else video.src = null;
        },

        takePicture: function takePicture(videoTagID, imageTagID) {
          if (!videoTagID || typeof videoTagID != 'string') return;

          if (!imageTagID || typeof imageTagID != 'string') return;
    ...
    util: function () {
      ...
      return {
        ...
        isImage: function isImage(file) {
          if (file && file.type) return file.type.substring(0, "image".length) == "image";
          return false;
        },

        // computeSize()
        computeSize: function computeSize(width, height) {
          // Keep the ratio
        ...
        // resizeImage()
        resizeImage: function resizeImage(img, callback, width, height) {

          // TODO : Diego : Do proper input checking (all functions)
          if (!FACE.util.isImage(img) || !callback || width <= 0 || height <= 0) return false;
          ...
          var readImgCallback = function readImgCallback(data) {
        ...
        // getImageFromInput()
        getImageFromInput: function getImageFromInput(inputFileID) {
          return document.getElementById(inputFileID).files[0];
        },

        // readFileAsBase64()
        readFileAsBase64: function readFileAsBase64(file, callback) {
          ...
          if (!file || !(file instanceof Blob) || !callback) return false;
        ...
        // displayImage()
        displayImage: function displayImage(imgFile, imgTagID, callback) {
          if (!imgTagID || typeof imgTagID != 'string') return;

          imgTag = document.getElementById(imgTagID);
          if (callback) imgTag.onload = callback();
          FACE.util.readFileAsBase64(imgFile, function (imgData) {
            imgTag.src = imgData;
          });
        },

        // resizeImageIfNeeded()
        resizeImageIfNeeded: function resizeImageIfNeeded(img, callback) {
